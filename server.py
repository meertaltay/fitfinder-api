...
_NOTIF_USERS = [
    ("Mira", "mira.vogue"), ("Berk", "berk.mode"), ("Zeynep", "zeynep.vip"),
    ("Can", "can.kaya"), ("Ece", "ece.drip"), ("Selin", "selin.vogue"),
    ("Defne", "defne.lux"), ("Ada", "ada.noir"), ("Sinan", "sinan.fit"),
]
_NOTIF_ITEMS_POOL = [
    ("Premium Kaşe Kaban", "4.200 TL", "massimodutti.com", "🧥"),
    ("Saten İpek Gömlek", "1.650 TL", "mango.com", "👔"),
    ("Bordo Süet Sneaker", "2.800 TL", "adidas.com", "👟"),
    ("Chelsea Bot", "2.900 TL", "drmartens.com", "👢"),
    ("Deri Bomber Ceket", "5.200 TL", "zara.com", "🧥"),
    ("Wide-Leg Keten", "1.100 TL", "cos.com", "👖"),
    ("Prada Çanta Muadili", "850 TL", "trendyol.com", "👜"),
    ("Chunky Altın Zincir", "1.200 TL", "hm.com", "📿"),
]
_NOTIF_FOLDERS = ["Sokak Stili", "Date Night", "Quiet Luxury", "Ofis Couture", "90s Grunge"]

@app.get("/api/notifications")
async def get_notifications(tab: str = "social"):
    """Premium notification feed — social or shopping tab."""
    _rand.seed(int(time.time() / 600))
    notifs = []

    if tab == "social":
        # 👑 Moda Otoritesi — Re-fitch bildirimleri
        item = _rand.choice(_NOTIF_ITEMS_POOL)
        others = _rand.sample(_NOTIF_USERS, 3)
        notifs.append({
            "type": "authority",
            "icon": "👑",
            "title": "Vizyonun İlham Veriyor!",
            "body": f"@{others[0][1]} ve {len(others)-1} kişi daha, dolabındaki '{item[0]}'ı kendi koleksiyonlarına Re-fitch'ledi.",
            "accent": "#D4AF37",
            "item_emoji": item[3],
            "count": _rand.randint(3, 12),
            "ago": f"{_rand.randint(5, 45)}dk önce",
            "cta": "Dolabını Gör",
        })

        # 📊 Vitrin Raporu
        score = _rand.randint(78, 96)
        ...
        notifs.append({
            ...
            "is_hof": is_hof,
            "ago": f"{_rand.randint(1, 3)}sa önce",
            "cta": "Vitrine Git",
        })

        # 👯 Stil İkizi
        twin = _rand.choice(_NOTIF_USERS)
        match_pct = _rand.randint(78, 95)
        notifs.append({
            "type": "style_twin",
            "icon": "📡",
            "title": "Stil DNA Eşleşmesi",
            "body": f"fitchy. Radarı: Seninle %{match_pct} oranında aynı stil DNA'sına sahip birini bulduk: @{twin[1]}. Dolabına sızmak ister misin?",
            "accent": "#e040fb",
            "match_pct": match_pct,
            "twin_handle": twin[1],
            "twin_avatar": _DEMO_AVATARS[_NOTIF_USERS.index(twin) % len(_DEMO_AVATARS)],
            "ago": f"{_rand.randint(1, 6)}sa önce",
            "cta": "Dolabını İncele",
        })

        # 👑 Küratör Takibi
        follower = _rand.choice(_NOTIF_USERS)
        folder = _rand.choice(_NOTIF_FOLDERS)
        notifs.append({
            "type": "curator_follow",
            "icon": "🍷",
            "title": "Küratörlüğün Takdir Ediliyor",
            "body": f"@{follower[1]}, senin '{folder}' klasörünü takibe aldı. Seçimlerine güveniyor.",
            "accent": "#D4AF37",
            "follower_handle": follower[1],
            "follower_avatar": _DEMO_AVATARS[_NOTIF_USERS.index(follower) % len(_DEMO_AVATARS)],
            "ago": f"{_rand.randint(10, 120)}dk önce",
            "cta": "Profilini Gör",
        })

        # Extra authority
        item2 = _rand.choice(_NOTIF_ITEMS_POOL)
        u2 = _rand.choice(_NOTIF_USERS)
        notifs.append({
            "type": "authority",
            "icon": "✨",
            "title": "Re-fitch Dalgası",
            "body": f"@{u2[1]}, senin keşfettiğin '{item2[0]}'dan ilham alarak kendi koleksiyonuna ekledi.",
            "accent": "var(--accent)",
            "item_emoji": item2[3],
            "count": 1,
            "ago": f"{_rand.randint(1, 90)}dk önce",
        ...
        # 💸 Fiyat Alarmı
        item = _rand.choice(_NOTIF_ITEMS_POOL)
        discount = _rand.choice([15, 20, 25, 30, 40])
        notifs.append({
            "type": "price_alert",
            "icon": "🚨",
            "title": f"%{discount} İndirim Fırsatı!",
            "body": f"Gözünün kaldığı o '{item[0]}'te %{discount} indirim var. Başkaları kapmadan önce dolabına git.",
            "accent": "#f44336",
            "item_name": item[0],
            "item_price": item[1],
            "item_emoji": item[3],
            "discount": discount,
            "ago": f"{_rand.randint(5, 30)}dk önce",
            "cta": "Fırsatı Yakala",
        })

        # ⏳ Stok Uyarısı
        item2 = _rand.choice(_NOTIF_ITEMS_POOL)
        stock = _rand.randint(1, 4)
        folder = _rand.choice(_NOTIF_FOLDERS)
        notifs.append({
            "type": "stock_alert",
            "icon": "⏳",
            "title": "Stok Alarmı!",
            "body": f"'{folder}' klasöründeki {item2[0]}'in senin bedenin için sadece {stock} stoğu kaldı!",
            "accent": "#ff9800",
            "item_name": item2[0],
            "item_emoji": item2[3],
            "stock": stock,
            "ago": f"{_rand.randint(10, 60)}dk önce",
            "cta": "Hemen Al",
        })

        # 🗝️ Muadil Keşfi
        brand = _rand.choice(["Prada", "Gucci", "Balenciaga", "Dior", "Bottega Veneta"])
        orig_price = _rand.choice(["25.000 TL", "35.000 TL", "48.000 TL", "62.000 TL"])
        dupe_price = _rand.choice(["650 TL", "850 TL", "1.200 TL", "1.450 TL"])
        notifs.append({
            "type": "dupe_found",
            "icon": "🗝️",
            "title": "Muadil Keşfedildi!",
            "body": f"Geçen hafta arattığın o {orig_price}'lik {brand} çantanın kusursuz bir muadili az önce fitchy. ağına düştü. Sadece {dupe_price}.",
            "accent": "#D4AF37",
            "brand": brand,
        ...
        notifs.append({
            "type": "ai_nudge",
            "icon": "🪞",
            "title": "Sanal Kabin Hazır",
            "body": "Merak ettiğin o triko üzerinde nasıl duracak? Sanal Kabin analizin hazır, hemen incele.",
            "accent": "var(--cyan)",
            "ago": f"{_rand.randint(2, 8)}sa önce",
            "cta": "Kabini Aç",
        })

        # Extra price alert
        item3 = _rand.choice(_NOTIF_ITEMS_POOL)
        notifs.append({
            "type": "price_drop",
            "icon": "📉",
            "title": "Fiyat Düşüşü Tespit Edildi",
            "body": f"Takip ettiğin '{item3[0]}' son 48 saatte {_rand.choice(['%12', '%18', '%25'])} ucuzladı. Şimdi {item3[1]}.",
            "accent": "#4caf50",
            "item_name": item3[0],
            "item_price": item3[1],
            "item_emoji": item3[3],
            "ago": f"{_rand.randint(30, 180)}dk önce",
            "cta": "Fiyatı Gör",
        })

    return {"success": True, "notifications": notifs, "tab": tab, "unread": _rand.randint(3, 8)}
...
.hof-scroll{display:flex;gap:12px;overflow-x:auto;padding-bottom:8px;margin-top:10px}
.hof-card{flex-shrink:0;width:140px;border-radius:18px;overflow:hidden;position:relative;cursor:pointer;border:2px solid rgba(255,215,0,.3);background:#1a1a2e;transition:transform .2s}
.hof-card:active{transform:scale(.95)}
.hof-card img{width:100%;height:180px;object-fit:cover;display:block}
.hof-card .hof-score{position:absolute;top:8px;right:8px;background:rgba(0,0,0,.8);color:#ffd700;font-size:11px;font-weight:800;padding:4px 8px;border-radius:8px;backdrop-filter:blur(4px);border:1px solid rgba(255,215,0,.3)}
.hof-card .hof-user{position:absolute;bottom:8px;left:8px;right:8px;font-size:11px;font-weight:700;color:#fff;text-shadow:0 1px 4px rgba(0,0,0,.8)}
...
.podyum-card .ac-name{font-size:24px;font-weight:900;color:#fff;letter-spacing:-.5px;text-shadow:0 2px 15px rgba(0,0,0,.8);pointer-events:auto;cursor:pointer;display:inline-block}
.podyum-card .ac-name:active{color:var(--cyan)}
...
/* 🔔 NOTIFICATION CENTER */
.notif-overlay{position:fixed;inset:0;z-index:300;background:rgba(5,2,10,.6);backdrop-filter:blur(8px);opacity:0;pointer-events:none;transition:opacity .3s}
.notif-overlay.show{opacity:1;pointer-events:all}
.notif-panel{position:fixed;top:0;right:-100%;width:100%;max-width:440px;height:100%;z-index:310;background:var(--bg);border-left:1px solid var(--border);display:flex;flex-direction:column;transition:right .35s cubic-bezier(.22,1,.36,1);box-shadow:-10px 0 40px rgba(0,0,0,.6)}
...
.notif-header .notif-title{font-size:18px;font-weight:900;color:var(--text)}
.notif-header .notif-close{width:32px;height:32px;border-radius:50%;background:rgba(255,255,255,.06);border:1px solid var(--border);display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:14px;color:var(--muted);transition:all .2s}
.notif-header .notif-close:hover{background:rgba(255,255,255,.1);color:var(--text)}
.notif-tabs{display:flex;padding:0 20px;gap:8px;border-bottom:1px solid rgba(255,255,255,.04)}
.notif-tab{flex:1;padding:12px 0;text-align:center;font-size:12px;font-weight:700;color:var(--muted);cursor:pointer;border-bottom:2px solid transparent;transition:all .2s;position:relative}
.notif-tab.active{color:var(--text);border-bottom-color:var(--accent)}
.notif-tab .tab-badge{position:absolute;top:6px;right:calc(50% - 40px);width:6px;height:6px;border-radius:50%;background:var(--accent)}
.notif-list{flex:1;overflow-y:auto;padding:12px 16px 100px;display:flex;flex-direction:column;gap:2px}
.notif-item{display:flex;gap:12px;padding:16px 14px;border-radius:16px;background:rgba(255,255,255,.02);border:1px solid var(--border);cursor:pointer;transition:all .2s;position:relative;overflow:hidden}
.notif-item:hover{background:rgba(255,255,255,.04);border-color:rgba(255,255,255,.08)}
.notif-item::before{content:'';position:absolute;top:0;left:0;width:3px;height:100%;border-radius:0 2px 2px 0}
.notif-icon{width:44px;height:44px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0}
.notif-content{flex:1;min-width:0}
.notif-content .ntitle{font-size:13px;font-weight:800;color:var(--text);margin-bottom:3px;display:flex;align-items:center;gap:6px}
.notif-content .nbody{font-size:12px;line-height:1.5;color:rgba(255,255,255,.7);margin-bottom:8px}
.notif-content .nbody b{color:var(--text);font-weight:700}
.notif-content .nbody .hl{font-weight:800}
...
.notif-twin-av{width:32px;height:32px;border-radius:50%;object-fit:cover;border:2px solid var(--border)}
/* Unread dot */
.notif-bell-dot{position:absolute;top:-2px;right:-2px;width:8px;height:8px;border-radius:50%;background:var(--accent);border:2px solid var(--bg);animation:bellPulse 2s infinite}
...
<body>
<div class="app">
  <div id="cameraScreen" style="padding:0 20px;position:relative;display:none">
    <div style="position:absolute;top:12px;left:0;z-index:10;cursor:pointer;font-size:13px;font-weight:600;color:var(--muted)" onclick="navTo('kesf')">← Keşfet</div>
    <div style="position:absolute;top:12px;right:0;z-index:10;cursor:pointer;font-size:20px" onclick="openNotifPanel()"><span style="position:relative">🔔<div class="notif-bell-dot" id="notifBellDotCamera"></div></span></div>
  ...
  <!-- ✧ KEŞFET (Global Vitrin & Trend Motoru) -->
  <div id="kesfScreen" style="display:none;padding:0;position:relative">
    <div style="position:sticky;top:0;z-index:40;background:rgba(5,2,10,.9);backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);padding:16px 20px;border-bottom:1px solid var(--border)">
    ...
    <div style="padding:20px">
      ...
      <div id="hofContainer" style="margin-bottom:28px">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px">
          <div style="font-size:16px;font-weight:800;display:flex;align-items:center;gap:8px"><span style="font-size:20px">🏆</span><span style="color:var(--gold)">90+ Kulübü Üyeleri</span></div>
        </div>
        <div id="kesfHofScroll" class="hof-scroll">
          <div class="loader-orb" style="width:30px;height:30px"></div>
        </div>
        <p style="font-size:11px;color:var(--muted);text-align:center;margin-top:10px">↑ Bir kullanıcının üzerine tıklayarak profiline sız! ↑</p>
      </div>

      <!-- 🔥 Trend Olan Stiller -->
      <div style="margin-bottom:28px">
        <div style="font-size:16px;font-weight:800;margin-bottom:14px;display:flex;align-items:center;gap:8px"><span style="font-size:20px">🔥</span>Şu An Trend</div>
        <div id="kesfTrends" style="display:flex;gap:12px;overflow-x:auto;padding-bottom:8px">
        </div>
      </div>
      ...
      <div style="margin-bottom:28px">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
          <div style="border-radius:16px;overflow:hidden;position:relative;height:200px;cursor:pointer;border:1px solid var(--border)" onclick="alert('Koleksiyon: Sessiz Lüks')">
            <img src="https://images.unsplash.com/photo-1490481651871-ab68de25d43d?w=400" style="width:100%;height:100%;object-fit:cover;opacity:.7">
            <div style="position:absolute;inset:0;background:linear-gradient(transparent 40%,rgba(5,2,10,.95));display:flex;flex-direction:column;justify-content:flex-end;padding:14px">
              <div style="font-size:9px;color:var(--cyan);font-weight:800;letter-spacing:1px;margin-bottom:4px">KOLEKSİYON</div>
              <div style="font-size:14px;font-weight:800;color:#fff">Sessiz Lüks</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">12 parça • Kürasyon</div>
            </div>
          </div>
          <div style="border-radius:16px;overflow:hidden;position:relative;height:200px;cursor:pointer;border:1px solid var(--border)" onclick="alert('Koleksiyon: Kış 2025')">
            <img src="https://images.unsplash.com/photo-1544957992-20514f595d6f?w=400" style="width:100%;height:100%;object-fit:cover;opacity:.7">
            <div style="position:absolute;inset:0;background:linear-gradient(transparent 40%,rgba(5,2,10,.95));display:flex;flex-direction:column;justify-content:flex-end;padding:14px">
              <div style="font-size:9px;color:var(--accent);font-weight:800;letter-spacing:1px;margin-bottom:4px">SEZON</div>
              <div style="font-size:14px;font-weight:800;color:#fff">Kış 2025</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">8 parça • Trend</div>
            </div>
          </div>
          <div style="border-radius:16px;overflow:hidden;position:relative;height:200px;cursor:pointer;border:1px solid rgba(255,215,0,.3)" onclick="alert('Koleksiyon: Sokak Stili')">
            <img src="https://images.unsplash.com/photo-1509631179647-0177331693ae?w=400" style="width:100%;height:100%;object-fit:cover;opacity:.7">
            <div style="position:absolute;inset:0;background:linear-gradient(transparent 40%,rgba(5,2,10,.95));display:flex;flex-direction:column;justify-content:flex-end;padding:14px">
              <div style="font-size:9px;color:var(--gold);font-weight:800;letter-spacing:1px;margin-bottom:4px">VİTRİN</div>
              <div style="font-size:14px;font-weight:800;color:#fff">Sokak Stili</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">15 parça • Global</div>
            </div>
          </div>
          <div style="border-radius:16px;overflow:hidden;position:relative;height:200px;cursor:pointer;border:1px solid var(--border)" onclick="alert('Koleksiyon: Y2K')">
            <img src="https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=400" style="width:100%;height:100%;object-fit:cover;opacity:.7">
            <div style="position:absolute;inset:0;background:linear-gradient(transparent 40%,rgba(5,2,10,.95));display:flex;flex-direction:column;justify-content:flex-end;padding:14px">
              <div style="font-size:9px;color:var(--purple);font-weight:800;letter-spacing:1px;margin-bottom:4px">NOSTALJI</div>
              <div style="font-size:14px;font-weight:800;color:#fff">Y2K Geri Döndü</div>
              <div style="font-size:10px;color:var(--muted);margin-top:2px">10 parça • Retro</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Podyum CTA -->
      <div onclick="navTo('podyum')" style="padding:18px;border-radius:18px;background:rgba(0,229,255,.06);border:1px solid rgba(0,229,255,.2);cursor:pointer;text-align:center;margin-bottom:80px">
        <div style="font-size:28px;margin-bottom:8px">✨</div>
        <div style="font-size:15px;font-weight:800;color:#fff">Podyuma Git</div>
        <div style="font-size:12px;color:var(--cyan);margin-top:4px">Stilleri incele, ilham ver veya pas geç</div>
      </div>
    </div>
  </div>
  ...
  <div class="bnav">
    ...
    <div class="bnav-item" id="nav-kesf" onclick="navTo('kesf')"><div class="icon">✧</div><div class="lbl">Keşfet</div></div>
    <div class="bnav-item bnav-logo" id="nav-logo" onclick="openCamera()"><div class="bnav-logo-ring"><span><img src="data:image/png;base64," style="width:90px;height:90px;border-radius:0;object-fit:contain"></span></div></div>
    <div class="bnav-item" id="nav-podyum" onclick="navTo('podyum')"><div class="icon">✨</div><div class="lbl">Podyum</div></div>
    <div class="bnav-item" id="nav-favs" onclick="navTo('favs')"><div class="icon">♡</div><div class="lbl">Profil</div></div>
  </div>
...
<div class="notif-panel" id="notifPanel">
  ...
  <div class="notif-tabs">
    <div class="notif-tab active" id="notifTabSocial" onclick="switchNotifTab('social')">✨ İlham & Sosyal<div class="tab-badge"></div></div>
    <div class="notif-tab" id="notifTabShop" onclick="switchNotifTab('shopping')">🛍️ Dolap Alarmları</div>
  </div>
...
<div class="story-modal" id="storyModal">
  <div style="padding:16px 20px;display:flex;align-items:center;justify-content:space-between;border-bottom:1px solid var(--border)">
    <div style="display:flex;align-items:center;gap:12px">
      <div style="width:44px;height:44px;border-radius:50%;padding:2px;background:var(--cyan);flex-shrink:0;cursor:pointer" onclick="var h=document.getElementById('storyHandle'),a=document.getElementById('storyAvatarImg');if(h)openPubProfile(h.textContent,a?a.src:'')"><img src="" id="storyAvatarImg" style="width:100%;height:100%;border-radius:50%;object-fit:cover;border:2px solid var(--bg)"></div>
      <div><div style="font-size:14px;font-weight:800;color:#fff;cursor:pointer" id="storyHandle" onclick="var a=document.getElementById('storyAvatarImg');openPubProfile(this.textContent,a?a.src:'')">@fitchy_user</div><div style="font-size:11px;color:var(--cyan);font-weight:600;margin-top:2px">✨ <span id="storyStatusTxt">Podyumda İlham Veriyor</span></div></div>
    </div>
...
document.getElementById('kesfScreen').style.display='block';
document.getElementById('nav-kesf').classList.add('active');
loadKesfContent();
...
function _closeAllScreens(){
  ...
  document.getElementById('pubProfileScreen').classList.remove('show');
  document.body.style.overflow='';
  document.getElementById('cameraScreen').style.display='none';
  document.getElementById('kesfScreen').style.display='none';
...
function openPubProfile(handleStr,avatarSrc){
  ...
  var p=MOCK_PROFILES[cleanHandle];
  if(!p){
...
function loadHOF_kesf(){
  fetch('/api/podyum-top?limit=10').then(function(r){return r.json()}).then(function(d){
    var c=document.getElementById('kesfHofScroll');if(!c)return;
    if(!d.ranked||d.ranked.length===0){
      c.innerHTML='<div class="hof-card" onclick="openPubProfile(\'mertaltay\',\'https://images.unsplash.com/photo-1516826957135-700ede19c6ce?w=400\')"><img src="https://images.unsplash.com/photo-1516826957135-700ede19c6ce?w=400"><div class="hof-score">🔥 94</div><div class="hof-user">@mertaltay</div></div><div class="hof-card" onclick="openPubProfile(\'ece.drip\',\'https://images.unsplash.com/photo-1551028719-0c124a4234c4?w=400\')"><img src="https://images.unsplash.com/photo-1551028719-0c124a4234c4?w=400"><div class="hof-score">👑 96</div><div class="hof-user">@ece.drip</div></div>';
      return;
    }
    var h='';
    d.ranked.forEach(function(e){
      var src=e.image_url||(e.image?(e.image.startsWith('http')?e.image:'data:image/jpeg;base64,'+e.image):'');
      var nick=(e.nickname||'anonim').replace(/\s+/g,'').toLowerCase();
      h+='<div class="hof-card" onclick="openPubProfile(\''+nick+'\',\''+src+'\')">';
      h+='<img src="'+src+'" onerror="this.src=\'data:image/svg+xml,<svg xmlns=http://www.w3.org/2000/svg viewBox=0 0 100 100><rect fill=%231a1a2e width=100 height=100/></svg>\'">';
      h+='<div class="hof-score">'+(e.emoji||'✨')+' '+e.ai_score+'</div>';
      h+='<div class="hof-user">@'+nick+'</div>';
      h+='</div>';
    });
    c.innerHTML=h;
  }).catch(function(){});
}
function loadTrending_kesf(){
  /* Trending mock — these would come from API later */
  var trends=[
  ...
  fetch('/api/radar-feed?page='+_radarPage+'&limit=10').then(function(r){return r.json()}).then(function(d){
    ...
    for(var i=0;i<d.cards.length;i++){
      ...
      if(c.type==='moodboard_drop'){
        ...
        h+='<div class="rcard-action"><button class="rbtn rbtn-primary" onclick="openPubProfile(\''+c.handle+'\')">\uD83D\uDC40 Profili G\u00F6r</button><button class="rbtn rbtn-secondary">\uD83D\uDCCC Kaydet</button></div>';
      }
...
function _avatarHead(url,name,handle,ago,colors){
  ...
  var h='<div class="rcard-head" style="cursor:pointer" onclick="openPubProfile(\''+handle+'\',\''+(url||'').replace(/'/g,"\\'")+'\')">';
...
/* ── 🔔 NOTIFICATION CENTER ── */
var _notifCurrentTab='social';
function openNotifPanel(){
  document.getElementById('notifOverlay').classList.add('show');
  document.getElementById('notifPanel').classList.add('show');
  document.getElementById('notifBellDot').style.display='none';
  loadNotifications(_notifCurrentTab);
}
...
function switchNotifTab(tab){
  ...
  document.getElementById('notifTabSocial').classList.toggle('active',tab==='social');
  document.getElementById('notifTabShop').classList.toggle('active',tab==='shopping');
  loadNotifications(tab);
}
function loadNotifications(tab){
  var list=document.getElementById('notifList');
  list.innerHTML='<div style="text-align:center;padding:40px"><div class="loader-orb" style="width:32px;height:32px;margin:0 auto 12px"></div><div style="color:var(--muted);font-size:12px">Y\u00FCkleniyor...</div></div>';
  fetch('/api/notifications?tab='+tab).then(function(r){return r.json()}).then(function(d){
    list.innerHTML='';
    if(!d.success||!d.notifications||!d.notifications.length){
      list.innerHTML='<div style="text-align:center;padding:60px 20px"><div style="font-size:36px;margin-bottom:12px">\uD83D\uDD14</div><div style="font-size:14px;font-weight:700;color:var(--text)">Hen\u00FCz bildirim yok</div><div style="font-size:12px;color:var(--muted);margin-top:4px">Aktivite olduk\u00E7a burada g\u00F6r\u00FCnecek.</div></div>';
      return;
    }
    for(var i=0;i<d.notifications.length;i++){
      ...
      else if(n.type==='ai_nudge') iconBg='rgba(0,229,255,.08)';
      else if(n.type==='price_drop') iconBg='rgba(76,175,80,.08)';

      h+='<div class="notif-icon" style="background:'+iconBg+'"><span>'+n.icon+'</span></div>';
      h+='<div class="notif-content">';
      h+='<div class="ntitle" style="color:'+n.accent+'">'+n.title+'</div>';
      ...
      if(n.type==='style_twin' && n.twin_avatar){
        h+='<div style="display:flex;align-items:center;gap:8px;margin:6px 0"><img class="notif-twin-av" src="'+n.twin_avatar+'" onerror="this.style.background=\'var(--accent)\'"><span style="font-size:11px;font-weight:700;color:var(--text)">@'+n.twin_handle+'</span><span style="font-size:10px;font-weight:800;color:#e040fb;margin-left:auto">%'+n.match_pct+' e\u015Fle\u015Fme</span></div>';
      }
      if(n.type==='dupe_found'){
        h+='<div style="display:flex;align-items:center;gap:8px;margin:6px 0;padding:8px;border-radius:10px;background:rgba(255,255,255,.02);border:1px solid var(--border)"><span style="font-size:11px;color:var(--muted);text-decoration:line-through">'+n.orig_price+'</span><span style="font-size:13px;font-weight:900;color:var(--accent)">'+n.dupe_price+'</span><span style="font-size:10px;font-weight:700;color:#4caf50;margin-left:auto">%'+Math.round((1-parseInt(n.dupe_price)/parseInt(n.orig_price.replace('.','')))*100)+' tasarruf</span></div>';
      }
      if(n.type==='price_alert' && n.discount){
        h+='<div style="display:inline-flex;align-items:center;gap:4px;padding:4px 8px;border-radius:8px;background:rgba(244,67,54,.1);border:1px solid rgba(244,67,54,.2);font-size:10px;font-weight:800;color:#f44336;margin:4px 0">-%'+n.discount+' \u0130ND\u0130R\u0130M</div>';
      }

      /* CTA button */
      var ctaColor=n.accent;
      ...
      h+='</div>';
      el.innerHTML=h;
      el.style.borderLeft='3px solid '+n.accent;
      list.appendChild(el);
    }
  }).catch(function(){
    list.innerHTML='<div style="text-align:center;padding:40px;color:var(--muted);font-size:12px">Ba\u011Flant\u0131 hatas\u0131</div>';
  });
}
function _formatNotifBody(text){
  /* Bold @handles and quoted items */
  return text.replace(/@(\w+[\.\w]*)/g,'<b style="color:var(--cyan)">@$1</b>')
    .replace(/'([^']+)'/g,'<b>\'$1\'</b>')
...
function renderPodyumStack(){
  ...
  for(var i=Math.min(_podyumIdx+2,_podyumCards.length-1);i>=_podyumIdx;i--){
    ...
    // Left: name + score
    h+='<div>';
    h+='<div class="ac-name" onclick="event.stopPropagation();openPubProfile(\''+nick+'\',\''+imgSrc.replace(/'/g,"\\'")+'\')">@'+nick;
...
function podyumDown(ev){
  if(ev.target.closest&&(ev.target.closest('.ac-name')||ev.target.closest('.report-btn-float')))return;
